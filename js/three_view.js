...
class ThreeViewer {
  ...
  update(data) {
    ...
    const numFloors =
      data.num_floors || +document.getElementById("num-floors").value || 5;
    ...
    for (let f = 0; f < numFloors; f++) {
      // Isolation check
      if (this.currentFloor !== "all" && parseInt(this.currentFloor) !== f)
        continue;
      ...
      if (data.rooms) {
        data.rooms.forEach((room) => {
          const isCore =
            room.unit_type === "stair" || room.unit_type === "elevator";
          ...
          if (!isDuct) {
            const ceilGeo = new THREE.ExtrudeGeometry(shape, {
              depth: 0.15,
              bevelEnabled: false,
            });
    ...
    if (group.children.length > 0 && !this.cameraLocked) {
      ...
      if (this.controls) {
        ...
        console.warn(
          "⚠️ [3D Detail] OrbitControls missing; using camera.lookAt fallback.",
        );
      ...
      if (this.controls) this.controls.update();
      console.log(
        `🎯 [3D Framing] Success. Center: (${center.x.toFixed(1)}, ${center.y.toFixed(1)}, ${center.z.toFixed(1)}) Dist: ${cameraDist.toFixed(1)}`,
      );
    }
  }
  ...
  _updateFloorButtons(count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (this.currentFloor !== "all" && parseInt(this.currentFloor) === i)
  ...
  _pointsToShape(points) {
    ...
    if (cleanPoints.length < 3) {
      console.error(
        "❌ [Geometry Error] Not enough valid points to form shape after NaN filtering.",
      );
      return shape;
    }
    ...
    for (let i = 1; i < cleanPoints.length; i++)
      shape.lineTo(cleanPoints[i][0], cleanPoints[i][1]);
    shape.closePath();
    return shape;
  }
  ...
  _updateLogOverlay(data, numFloors) {
    ...
    if (elArea)
      elArea.textContent =
        Math.round(area * numFloors).toLocaleString() + " m²";
  }
}
